    ...
    with col1:
        def generate_graph_callback():
            if file_link and ontology_link:
                ...
                st.session_state.graphs[0] = g1_l.serialize(format='ttl')
        ...
        st.button("Generate Graph", on_click=generate_graph_callback)
        
        tab1, tab2, tab3, tab4 = st.tabs(["Graph 1", "Graph 2", "Graph3", "Merged Graph"])

        with tab1:
            st.text(st.session_state.graphs[0])
        
        with tab2:
            st.text(st.session_state.graphs[1])
        
        with tab3:
            st.text(st.session_state.graphs[2])
        
        with tab4:
            st.text(st.session_state.graphs[3])
